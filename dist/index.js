...
async function createLead(lead) {
  ...
  try {
    ...
    const createdLead = await db.select().from(leads).orderBy((t2) => t2.id).limit(1);
    return createdLead.length > 0 ? createdLead[0] : null;
  } catch (error) {
...
var appRouter = router({
  ...
  // Lead submission for agent recruitment
  leads: router({
    submitLead: publicProcedure.input(z2.object({
      ...
      phone: z2.string().min(10, "Valid phone number is required"),
      experience: z2.string().min(1, "Experience level is required"),
...
function serveStatic(app) {
  const distPath = process.env.NODE_ENV === "development" ? path2.resolve(PROJECT_ROOT, "dist", "public") : path2.resolve(import.meta.dirname, "public");
